...
[estimatedMap,~]=mapEnvironments(resolution,scenario_estimate);
...
if wallAvoidance == false
    if outer_loop == 1
        ...
        if any(checkOccupancy(estimatedMap,poses_inverted(:,1:2)))
            idx = find(checkOccupancy(estimatedMap,...
                poses_inverted(:,1:2)) == 1);
            distance = sqrt((poses_inverted(idx,1) - endPoses(:,1)).^2+...
               (poses_inverted(idx,2) - endPoses(:,2)).^2);
            if find(distance==min(distance)) == size(endPoses,1)
                index = find(distance==min(distance));
            else
                index = find(distance==min(distance))+1;
            end
            goal_new = [endPoses(index,2) endPoses(index,1)];
            % switch to T2 algorithm:
            wallAvoidance = true;              
        end
    end
    
    
    % This section tells the robot to follow each point of the path
    if wallAvoidance == false
        ...
        if all(abs([cur_x cur_y] - goal_coordinate) < 0.1)
           if current_point == size(poses,1)
                break;
           else
