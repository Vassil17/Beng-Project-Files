function [psi_sg,psi_vg,subgoal,virtgoal,gap,closestGap]=TGF_algorithm_old(obstacleMap,...
    scan,cur_x,cur_y,angleToGoal,goal,cur_psi,range)
%
% CHECKING LEAVING CONDITION NEEDS TO TAKE INTO ACCOUNT RANGE
%
% This function contains the updated Tangential gap flow algorithm (TGF). 
% It searches for gaps between
...
% backward search
    ...
    if i == 1
        ...
        for gapNumber = 1:i-1
           ...
           gap(gapNumber).AngleDifference = abs(angleToGoal - (cur_psi - gap(gapNumber).ClosestGapAngle));
        ...
        idx = find([gap.Gap] == 0);
        gap(idx) = [];
        % If statement to check whether any gaps are accessible
        if ~all([gap.Gap]==0)
         noGaps = 0;
        ...
        else % if none are accessible / no gaps
          noGaps = 1;
        end
        if noGaps == 0
           ...
           index_theta_os = find((gap(closestGap).Sides(:,3) ~= theta_cs));
            ...
            psi_sg = 0;
        end % end noGaps if statement
    end % end i==1 if statement
    %
%
