...
%     err_vel(n) = desired_vel - cur_vel;
    ...
    Kp_vel = 15;    % stable but slow for Kp = 2; Ki = 0.01; Kd = 1;
    Ki_vel = 1;
    Kd_vel = 5;
...
%     err_vel_d(n) = (err_vel(n) - err_vel(prev_n))/dT;
    ...
    u_psi(n) = Kp_psi * err_psi(n) + Ki_psi * err_psi_i(n) +...
        + Kd_psi * err_psi_d(n) ;
%     u_vel(n) = Kp_vel * err_vel(n) + Ki_vel * err_vel_i(n) +...
%         + Kd_vel * err_vel_d(n);
...
%         if sign(err_vel(n)) == sign(u_vel(n))
%             err_vel_i(n) = 0;
%         end
%     end
    ...
    u_vel(n) = 2*pdist([desired_pt;cur_x,cur_y],'euclidean');
