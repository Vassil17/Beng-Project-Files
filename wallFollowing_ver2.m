function [desired_psi,state,stopRobot,originalPosition]=wallFollowing_ver2(objectDetected,...
    state,cur_psi,desired_psi,position,originalPosition,distance)
if abs(cur_psi - desired_psi)>2*pi
    desired_psi = cur_psi;
end
...
elseif state == 2
    if abs(cur_psi- desired_psi) < 0.05
        if objectDetected(1) == 1 && distance(1) < 0.8
            stopRobot=1;
            state=0;
