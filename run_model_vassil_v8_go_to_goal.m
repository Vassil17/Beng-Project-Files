...
wall{3} = WallGeneration1(0,0,6,8,'v');
...
for outer_loop = 1:(sim_time/dT)
    ...
    sensors = [cur_y cur_x; cur_y cur_x; cur_y cur_x;];
    % Set up angle for each sensor
    sensorAngle = [cur_psi; cur_psi - pi/2; cur_psi + pi/2];
...
%          state,cur_psi,desired_psi,position,originalPosition,distance,check_for_goal,scan);  
     [desired_psi,state,stopRobot,originalPosition]=wallFollowing_v3(objectDetected,...
